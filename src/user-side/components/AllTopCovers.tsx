...
const AllTopCovers: React.FC = () => {
  ...
  // Compute items per page based on viewport
  useEffect(() => {
    const compute = () => {
      const width = window.innerWidth;
      if (width >= 1024) return 12; // 4 cols * 3 rows
  ...
  if (loading) return <p className="text-center py-12 text-gray-600">Loading products...</p>;
  if (error) return <p className="text-center py-12 text-red-600">{String(error)}</p>;

  const getImage = (p: Product) => p.imageUrl || p.image || p.coverImage || "";
  const getTitle = (p: Product) => p.title || p.name || "Untitled";
  const getPrice = (p: Product) => (p.price != null ? Number(p.price) : NaN);
  const getOldPrice = (p: Product) => {
    const price = getPrice(p);
    return Number.isNaN(price) ? NaN : Math.round(price * 1.2);
  };
  const getDiscountPercent = (p: Product) => {
    const price = getPrice(p);
    const oldPrice = getOldPrice(p);
    return Number.isNaN(price) || Number.isNaN(oldPrice) || oldPrice <= price
      ? 0
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 text-gray-900 dark:text-gray-100">
      <h1 className="text-2xl sm:text-3xl font-bold mb-6">All Top Covers</h1>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {paginatedProducts.map(product => (
              <div
                key={String(product.id ?? Math.random())}
                className="group bg-white dark:bg-gray-800 border rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden"
              >
                <div className="relative aspect-[4/5] bg-gray-50 dark:bg-gray-700 overflow-hidden">
                  {getDiscountPercent(product) > 0 && (
                    <span className="absolute left-3 top-3 z-10 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow">
                  ...
                  <button
                    onClick={e => {
                      e.preventDefault();
                      if (product.id != null) navigate(`/user/product/${product.id}`);
                    }}
                    className="absolute inset-x-3 bottom-3 z-10 flex items-center justify-center gap-2 bg-white/95 text-gray-800 border border-gray-200 rounded-full py-2 text-xs font-medium shadow-md opacity-100 sm:opacity-0 sm:translate-y-2 sm:group-hover:opacity-100 sm:group-hover:translate-y-0 transition-all duration-300"
                  >
