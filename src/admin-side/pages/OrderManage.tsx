...
const OrderManage: React.FC = () => {
  ...
  const { orders, loading, error } = useSelector(
    (s: RootState) => (s as any).adminOrders || { orders: [], loading: false, error: null }
  );
      ...
      <html>
        ...
        <body onload="window.print();window.close()">
          <div class="slip">
            ...
            <div class="row"><span class="label">Order Date</span><span class="value">${
              o.createdAt ? new Date(o.createdAt).toLocaleString() : "-"
            }</span></div>
            <div class="row"><span class="label">Status</span><span class="value">${o.status}</span></div>
            <div class="row"><span class="label">Customer</span><span class="value">${userName}</span></div>
            <div class="line"></div>
            <div class="row"><span class="label">Ship To</span><span class="value" style="text-align:right">${addressLines ||
              "-"}</span></div>
            <div class="row"><span class="label">Payment</span><span class="value">${paymentMethod}</span></div>
            <div class="line"></div>
            <div class="row"><span class="label">Total Items</span><span class="value">${totalItems}</span></div>
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6">
      <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
        <h1 className="text-lg sm:text-xl md:text-2xl font-bold">All Orders</h1>
      ...
      <div className="mt-4 overflow-x-auto bg-white border border-gray-200 rounded-lg sm:rounded-xl shadow-sm">
        {loading ? (
          ...
          <>
            ...
            <div className="sm:hidden divide-y divide-gray-200">
              {paginatedOrders.map((o: any) => (
                <div key={o.id} className="p-3 hover:bg-gray-50">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <p className="text-sm font-medium">Order #{o.id}</p>
                      <p className="text-xs text-gray-600">{o.User?.name || o.user?.name || o.userId}</p>
                      <p className="text-xs text-gray-600">{o.User?.email || o.user?.email || "-"}</p>
                    </div>
                  ...
                  <div className="mt-2">
                    <div className="flex flex-col gap-1">
                      <select
                        ...
                        value={o.status}
                        onChange={(e) =>
                          dispatch(updateOrderStatusThunk({ id: o.id, status: e.target.value })).then(() =>
                            dispatch(fetchAllOrders())
                          )
                        }
                      >
                  ...
                  <div className="mt-2">
                    <button
                      onClick={() => setSelected(o)}
                      className="w-full text-center py-1 text-xs border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50"
                    >
                      View Details
                    </button>
                  </div>
            ...
            <table className="hidden sm:table min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">Order #</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">User</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">Email</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">Total</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">Status</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">Items</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {paginatedOrders.map((o: any) => (
                  <tr key={o.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm">{o.id}</td>
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm">{o.User?.name || o.user?.name || o.userId}</td>
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm">{o.User?.email || o.user?.email || "-"}</td>
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-medium">Rs{o.totalAmount}</td>
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                        <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold bg-blue-50 text-blue-700">{o.status}</span>
                        <select
                          className="border rounded px-2 py-1 text-xs sm:text-sm"
                          value={o.status}
                          onChange={(e) =>
                    ...
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm max-w-[120px]">
                      <div className="max-h-20 overflow-auto pr-1 space-y-1">
                        {(o.OrderItems || []).map((it: any) => (
                    ...
                    <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm">
                      <button
                        onClick={() => setSelected(o)}
                        className="inline-flex items-center rounded-md border border-gray-300 px-2 sm:px-3 py-1 text-xs sm:text-sm text-gray-700 hover:bg-gray-50"
                      >
                        View
                      </button>
                    </td>
            ...
            {/* Pagination */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-3 sm:px-4 py-3 border-t gap-2 sm:gap-0">
              <p className="text-xs sm:text-sm text-gray-600">Page {page} of {totalPages} — {(orders?.length || 0)} results</p>
              <div className="inline-flex items-center gap-2">
                <button
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  disabled={page <= 1}
                  className={`px-2 sm:px-3 py-1 rounded border text-xs sm:text-sm ${page <= 1 ? "text-gray-400 border-gray-200 bg-gray-50 cursor-not-allowed" : "text-gray-700 border-gray-300 hover:bg-gray-50"}`}
                >
                  Previous
                </button>
                <button
                  onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
                  disabled={page >= totalPages}
                  className={`px-2 sm:px-3 py-1 rounded border text-xs sm:text-sm ${page >= totalPages ? "text-gray-400 border-gray-200 bg-gray-50 cursor-not-allowed" : "text-gray-700 border-gray-300 hover:bg-gray-50"}`}
                >
                  Next
                </button>
              </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
          <div className="absolute inset-0 bg-black/40" onClick={() => setSelected(null)} />
          <div className="relative z-10 w-full max-w-md sm:max-w-lg md:max-w-2xl rounded-xl sm:rounded-2xl bg-white p-4 sm:p-6 shadow-xl overflow-auto max-h-[90vh]">
            <div className="flex items-start justify-between">
              <h3 className="text-lg font-semibold">Order Details</h3>
              <button onClick={() => setSelected(null)} className="rounded p-1 text-gray-500 hover:bg-gray-100">✕</button>
            </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3">
              <button onClick={() => printSlip(selected)} className="rounded-md border px-3 sm:px-4 py-2 text-xs sm:text-sm hover:bg-gray-50">Download Slip / Print</button>
              <button onClick={() => setSelected(null)} className="rounded-md bg-gray-800 px-3 sm:px-4 py-2 text-white text-xs sm:text-sm hover:bg-black">Close</button>
            </div>
          </div>
        </div>
      )}
